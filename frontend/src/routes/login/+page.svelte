...
<div class="card bg-base-300 w-[80%] m-auto mt-20 p-5 prose text-center">
  ...
  <div class="flex flex-row align-middle justify-center w-full">
    {#each data.authMethods.authProviders as method}
      <div class="mx-5">
        <Button variant="fill" class="btn btn-xl variant-filled-primary rounded-lg shadow-md font-bold capitalize" on:click={() => gotoAuthProvider(method)}>{method.name}</Button>
      </div>
    {/each}
  </div>
</div>